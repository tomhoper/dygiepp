...
if __name__ == '__main__':
    ...
    if args.mech_effect_mode == True:
        ...
        serial_dir = pathlib.Path(args.root) / 'experiments' / args.data_combo / 'finetune' / 'mapped' / 'mech_effect'
    if args.mech_effect_mode == False:
        data_root = pathlib.Path(args.root) / data_root_dir / args.data_combo / 'mapped' / 'mech'
        serial_dir = pathlib.Path(args.root) / 'experiments' / args.data_combo / 'finetune' / 'mapped' / 'mech'
    ...
    allennlp_command = [
            ...
            "fine-tune",
            "--model-archive",
            "/data/aida/covid_clean/experiments/scierc/mapped/mech/",
            "--config-file",
            config_file,
            "--serialization-dir",
            str(serial_dir),
